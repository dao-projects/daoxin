...
// 读取配置文件
const readJSONFile = (file) => JSON.parse(readFileSync(cwdResolve(file), "utf-8"));
// 写入配置文件
...
const versionIncrement = (type = "") => {
    ...
    if (isProd) {
        ...
        pkg.homepage = `${pkg?.baseConfig?.cdn || "https://cdn.dpfrc.com/"}${currentPath}/`;
...
const cssLoaderConfig = [cssLoader.styleLoader, cssLoader.cssLoader, cssLoader.postcssLoader];
...
const webpackBaseConfig = {
    ...
    plugins: [
        new HtmlWebpackPlugin({
            title: `${pkg?.title || pkg?.baseConfig?.title || "~~首页~~"} ${isDev ? " | 开发环境" : ""}`,
        ...
        // OSS 文件上传
        isProd && pkg?.baseConfig?.oss?.accessKeyId && new daoAliyunOssWebpackPlugin(pkg?.baseConfig?.oss),
    ].filter(Boolean),
    resolve: {
        extensions: [".ts", ".js", ".tsx", ".jsx", ".json", ".css", ".less", ".scss"],
        alias: {
            "@": cwdResolve("src"),
